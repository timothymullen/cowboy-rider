...
import horse_pic from './assets/horse.png'
...
let sketch = (p) => {
  ...
  p.keyPressed = () => {
    if (p.keyCode === p.LEFT_ARROW) {
      if (context_switcher.state === 'start') {
        context_switcher.start_frame = p.frameCount;
      }
  ...
  class Path {
    ...
    draw() {
      ...
      const frame_counter  = Math.floor((p.frameCount - context_switcher.start_frame) / 1.6)
  ...
  class Cowboy {
    ...
    blit(x, y) {
      p.image(this.image, x - (this.image.width / 2), y - (this.image.height / 2))
    }
  }
